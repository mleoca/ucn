...
const COMMAND_CONTRACTS = Object.freeze({
    ...
    usages: contract({
        ...
        purpose: 'Inventory definitions, imports, calls, and references without pretending every occurrence binds to one target.',
        ...
        jsonData: 'Array of definition and occurrence records with usage type, file, line, and content.',
    ...
    search: contract({
        ...
        defaults: ['Text term is regex unless `regex=false`.', 'Case-insensitive unless `caseSensitive=true`.'],
    ...
    plan: contract({
        ...
        modes: [
            { name: 'rename', when: '`renameTo` is supplied.', answer: 'Definition-related call/import edit previews.' },
            { name: 'add parameter', when: '`addParam` is supplied.', answer: 'Signature and call-site preview, optionally with a default.' },
            { name: 'remove parameter', when: '`removeParam` is supplied.', answer: 'Signature and affected call-site preview.' },
        ],
        defaults: ['Preview only; no file writes.'],
        truth: 'Changes are derived from indexed definitions/usages and retain unverified/blocked evidence separately.',
        ...
        jsonData: 'Before/after signatures, concrete change previews, unverified sites, and account metadata.',
    ...
    deadcode: contract({
        ...
        truth: 'A result means no modeled usage survived the command policy; it is explicitly not safe-delete proof.',
        nonGoals: ['External consumers, reflection, generated registration, or runtime reachability proof.'],
        invalidCombinations: [],
        examples: ['ucn deadcode --exclude=test', 'ucn deadcode --include-exported --limit=100'],
        jsonData: 'Array of candidate symbol records with location, visibility, and modeled usage count.',
